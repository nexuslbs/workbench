/**
 * CORE credentials provider `user-env`: the user level env file, i.e.
 ...
 * user's home directory (`$HOME`, then the OS answer); an explicit `file` wins
 * over `dir`. A missing file answers "not found"; a key is looked up exactly,
 * then as its ENV-normalised form (`demo-token` -> `DEMO_TOKEN`). `ref.scope`
 * is ignored.
 */
