/**
 * CORE credentials provider `project-env`: the project level env file, i.e.
 * `<projectDir>/.env` (the project scope).
 *
 * Semantics (documented in `docs/CREDENTIALS.md`): the file defaults to
 * `<dir>/.env` where `dir` is the plugin's configured `dir`, defaulting to the
 * config file directory (the project root); an explicit `file` wins over `dir`.
 * A missing file answers "not found"; a key is looked up exactly, then as its
 * ENV-normalised form (`demo-token` -> `DEMO_TOKEN`). `ref.scope` is ignored.
 */
import path from 'node:path'
import type { Context } from 'cordis'
import { type CredentialsService } from '../definition.ts'
import type { CredentialProvider } from '../definition.ts'
...
export function apply(ctx: Context & { credentials: CredentialsService }, config: Config, configDir: string = process.cwd()): void {
