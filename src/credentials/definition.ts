...
export abstract class CredentialsService extends Service {
  ...
  /**
   * Git auth strategies by `auth.type`, registered by PLUGIN-provided git auth
   * handlers (e.g. `github-app`). The core implements NONE of them: this map is
   * the extension point that keeps backend-specific minting out of the kernel.
   */
  protected gitAuthHandlers = new Map<string, GitAuthHandler>()
...
export interface CredentialConsumer {
  ...
  /** Enabled provider ids, in precedence order. */
  enabled(): string[]
  /** A git auth strategy registered for `auth.type` by a plugin, or undefined. */
  gitAuth?(type: string): GitAuthHandler | undefined
  /** The `auth.type`s plugins registered a git auth strategy for (sorted). */
  gitAuthTypes?(): string[]
}
