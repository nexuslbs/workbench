 ...
 *
 * - PROVIDERS implement the serving side. The core ships exactly one
 *   (`./providers/http.ts`, a `node:http` server wired by the composition root
 *   `../kernel.ts`); it is the only role that touches a socket. A provider
 ...
 * provider (or the composition root) may import a provider module, and the
 * definition imports no sibling module of this package.
 */
 ...
 */
export class Web extends Service {
  ...
  constructor(ctx: Context, name: string = WEB) {
    super(ctx, name)
  }
  ...
  asset(spec: WebAssetSpec): () => void {
    ...
    this.owners.set(`asset:${path}`, applyingPlugin())
    return () => {
  ...
  page(spec: WebPageSpec): () => void {
    ...
    this.owners.set(`page:${spec.id}`, applyingPlugin())
    return () => {
  ...
  ownerOf(key: string): string {
    return this.owners.get(key) ?? 'core'
  }
...
/**
 * Typed handle for every consumer/provider module: `ctx.web`. Consumers import
 * the DEFINITION (never a provider) and get full typing from this.
 */
