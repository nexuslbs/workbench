...
```sh
docker build -t wb-browser:local deploy/browser
docker run --rm -p 127.0.0.1:9222:9222 wb-browser:local
curl -fsS http://127.0.0.1:9222/json/version    # {"Browser":"Chrome/153...."}
```

`.github/workflows/publish.yml` publishes the SAME Dockerfile as
`ghcr.io/nexuslbs/workbench-browser:X.Y.Z` (and `:latest`) on a `vX.Y.Z` tag -
alongside, and independently of, the core image. `deploy/browser/docker-compose.yml`
runs it as its own compose project:

```sh
docker compose -f deploy/browser/docker-compose.yml -p workbench-browser up -d
```
...
plugins:
  ...
  browser-use-playwright:
    browserService:
      ...
      image: ghcr.io/nexuslbs/workbench-browser:0.0.8
